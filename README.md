...
- **网络隔离（deny-by-default）**：会话容器位于 internal 私有网（无任何直连外联），唯一出口是
  出网代理；代理只放行公网 + 控制平面，**拒绝一切私网 / 云元数据（169.254）**。runner 纯出站
  （长轮询拉取消息，控制平面不连入容器），杜绝 SSRF / 横向移动 / 偷云凭证。
  浏览器经代理上网（注：CloakBrowser 在严格代理下的截图链路仍在调试，agent 会优雅回退到 web_fetch）。
